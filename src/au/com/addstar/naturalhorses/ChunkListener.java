...
public class ChunkListener implements Listener {
	...
	public void onChunkLoad(ChunkLoadEvent event) {
		// Ignore newly generated chunks
		if (event.isNewChunk()) { return; }

		// Ignore worlds not in the list
		World world = event.getWorld();
		if (!NaturalHorses.HorseWorlds.contains(world.getName())) { return; }
		...
		if ((NaturalHorses.RandomGen.nextDouble() * 100) < NaturalHorses.SpawnChance) {
			...
			// Biome checks
			if (!NaturalHorses.HorseBiomes.contains(chunkdata.getBiome(0, 0).name())) 	{ return; }	// 
			if (!NaturalHorses.HorseBiomes.contains(chunkdata.getBiome(15, 0).name())) 	{ return; } //  This is not the biome you are looking for.  
			if (!NaturalHorses.HorseBiomes.contains(chunkdata.getBiome(0, 15).name())) 	{ return; } //  You will go home and reload your chunks. 
			if (!NaturalHorses.HorseBiomes.contains(chunkdata.getBiome(15, 15).name())) { return; } //
